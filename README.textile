...
After you have the gem installed, add a @config.gem@ line to your @environment.rb@ file:
  ...
  config.gem 'bouncy_bots', :version => 0.1.0

h2. Using Bouncy Bots!

h3. Controllers

Use the @bounce_bot@ macro in your controllers to detect and bounce bots.  The macro takes two parameters, the honey pot field name and the redirect path or url.  For example, to check for the field @:blog_url@ and redirect bots to the @pages_path@ :

  bounce_bot :blog_url, :pages_path

You can also pass the standard controller filter options such as @:only@ or @:except@ :

  bounce_bot :blog_url, :pages_path, :only => [:create, :update]
...
If you use @form_for@, there are two helpers to simplify making the honey pot.
  ...
  %br
  = f.bounce_field :class => 'highlight_required'
